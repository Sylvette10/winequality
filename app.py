...
<style>
    ...
    .main-container {padding: 2rem; max-width: 1000px; margin: 0 auto;}
    .header-section {text-align: center; margin-bottom: 3rem; padding: 2rem; background: linear-gradient(135deg, #8B0000 0%, #A52A2A 100%); border-radius: 15px; color: white;}
    .header-title {font-size: 2.5rem; font-weight: bold; margin-bottom: 0.5rem;}
    .header-subtitle {font-size: 1.2rem; opacity: 0.9; margin-bottom: 1rem;}
    .stats-container {display: flex; justify-content: space-around; margin: 2rem 0; gap: 1rem;}
    .stat-box {background: #FAF3E0; padding: 1.5rem; border-radius: 10px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1); flex: 1;}
    .stat-number {font-size: 2rem; font-weight: bold; color: #8B0000;}
    .stat-label {color: #666; font-size: 0.9rem;}
    .input-section {background: white; padding: 2rem; border-radius: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 2rem;}
    .result-box {padding: 2rem; border-radius: 15px; text-align: center; margin: 2rem 0; border: 3px solid;}
    .result-good {background: linear-gradient(135deg, #8B0000 0%, #A52A2A 100%); border-color: #8B0000; color: white;}
    .result-standard {background: linear-gradient(135deg, #E63946 0%, #A52A2A 100%); border-color: #E63946; color: white;}
    .result-title {font-size: 2rem; font-weight: bold; margin-bottom: 1rem;}
    .confidence-text {font-size: 1.5rem; margin-bottom: 1rem;}
    .stNumberInput > div > div > input {background-color: #f8f9fa; border: 2px solid #e9ecef; border-radius: 8px; padding: 0.5rem;}
    .stNumberInput > div > div > input:focus {border-color: #8B0000; box-shadow: 0 0 0 0.2rem rgba(139, 0, 0, 0.25);}
    .stButton > button {background: linear-gradient(135deg, #8B0000 0%, #A52A2A 100%); color: white; border: none; border-radius: 25px; padding: 0.75rem 2rem; font-size: 1.1rem; font-weight: bold; transition: all 0.3s ease;}
    .stButton > button:hover {transform: translateY(-2px); box-shadow: 0 8px 15px rgba(139, 0, 0, 0.4);}
    .stTabs [data-baseweb="tab-list"] {gap: 8px;}
    .stTabs [data-baseweb="tab"] {height: 50px; white-space: pre-wrap; background-color: #FAF3E0; border-radius: 10px; color: #8B0000; font-weight: bold;}
    .stTabs [aria-selected="true"] {background: linear-gradient(135deg, #8B0000 0%, #A52A2A 100%); color: white;}
</style>
...
def main():
    ...
    <div class="stats-container">
        <div class="stat-box"><div class="stat-number">{model_info.get('accuracy', 0):.1%}</div><div class="stat-label">Model Accuracy</div></div>
        <div class="stat-box"><div class="stat-number">{len(feature_names)}</div><div class="stat-label">Chemical Parameters</div></div>
        <div class="stat-box"><div class="stat-number">{model_info.get('model_name', 'ML')}</div><div class="stat-label">Algorithm Used</div></div>
        <div class="stat-box"><div class="stat-number">{model_info.get('quality_threshold', 7)}</div><div class="stat-label">Quality Threshold</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with st.form("wine_analysis"):
            ...
            with col2:
                analyze_button = st.form_submit_button("🔎 Analyze Wine Quality", use_container_width=True)
        if analyze_button:
            ...
            with col1:
                ...
                <div class="result-box {result_class}">
                    ...
                    <p>{'✅ Recommended for premium wine collection' if is_good else '⚠️ Suitable for standard wine offerings'}</p>
                </div>
                """, unsafe_allow_html=True)
                if confidence > 0.8:
    ...
    with tab2:
        ...
        with col1: st.metric("Accuracy", f"{model_info.get('accuracy', 0):.1%}")
        with col2: st.metric("Model Type", model_info.get('model_name', 'Unknown'))
        with col3: st.metric("Features", len(feature_names))
        with col4: st.metric("Threshold", model_info.get('quality_threshold', 7))
        st.markdown("---")
